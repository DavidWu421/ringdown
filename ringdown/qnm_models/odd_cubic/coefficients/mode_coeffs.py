...
def get_odd_cubic_coeffs_from_file(filename: Path):
    """
...
def get_odd_cubic_mode_coeffs(mode):
    """
    mode: [l, m, n]
    ...
    filename = folder / f"cubic_odd_{parity}_l{abs(l)}_m{m}_n{n}.txt"

    # Call the file-reading function
    return get_odd_cubic_coeffs_from_file(filename)

real_mode_coeffs=[]
imag_mode_coeffs=[]
for mode in mode_list:
    real_coeffs,imag_coeffs = get_odd_cubic_mode_coeffs(mode)
