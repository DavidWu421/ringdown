...
ai_omega = jnp.stack(a_omega, dtype=jnp.float64)
ai_gamma = jnp.stack(a_gamma, dtype=jnp.float64)


# Main function for computing omega/gamma values
def jnp_chiq_exact_factors(chi, ai):
...
# Main function for computing omega/gamma values
def compute_shifts(chi, ai, bi):
    poly_term = jnp.einsum('ij,jk->ik', aij, get_poly_chi_basis(chi))
    return poly_term


def get_cubic_omega_shifts(chi, alpha):
    omega = compute_shifts(chi, ai_omega).astype(jnp.complex64)
    gamma = compute_shifts(chi, ai_gamma).astype(jnp.complex64)
