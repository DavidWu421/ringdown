...
class Cubic:
    ...
    def __init__(self, modes):
        self.modes = modes
        self.cubic_modes = kerrmod.Kerr(modes)
        self.indices = []
    ...
    def r2_phi_to_alpha_chi(self, half_r_squared_alphachi, phi_alphachi):
        r = jnp.sqrt(2 * half_r_squared_alphachi)
        chi = r * jnp.cos(phi_alphachi)
        alpha = r * jnp.sin(phi_alphachi)
        return chi, alpha
    ...
    def prior_sample(self):
        variables = {}
        for var in ['m', 'half_r_squared_chialpha', 'phi_chialpha']:
        ...
        m = variables['m']
        half_r_squared_chialpha = variables['half_r_squared_chialpha']
        phi_chialpha = variables['phi_chialpha']

        r_chialpha = jnp.sqrt(2 * half_r_squared_chialpha)
        chi_value = r_chialpha * jnp.cos(phi_chialpha)
        alpha_value = r_chialpha * jnp.sin(phi_chialpha)

        chi = numpyro.deterministic("chi", chi_value)
        alpha = numpyro.deterministic("alpha", alpha_value)
