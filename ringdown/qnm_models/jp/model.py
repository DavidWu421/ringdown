...
class JP:
    ...
    def __init__(self, modes):
        ...
        for mode in self.modes:
            if mode in mode_to_index.keys():
                ...
                raise ValueError(
                    f"Don't have computed JP modes for mode {mode}")
    ...
    def get_jp_omega(self, chi, alpha):
        omega_kerr, gamma_kerr = self.get_kerr_omega(chi)
        omega = omega_kerr + alpha * compute_shifts(chi, self.aij_omega).flatten()
        # There's a minus sign below because gamma_kerr is the minus of the imaginary part of the Kerr mode.a_gamma
        # The JP fits give straight up the imaginary part, so there's a sign difference.
    ...
    def get_charged_ftau(self, M, chi, alpha):
        ...
        MREF = 68.0
        f0 = FREF * MREF / M
        omega0, gamma0 = self.get_jp_omega(chi, alpha)
        omega_c = f0 * self.get_jp_omega(chi, alpha)
    ...
    def get_freqs_and_gammas(self, m, chi, alpha):
        f0 = 1 / (m * qnms.T_MSUN)
        f_gr_0, g_gr_0 = self.get_jp_omega(chi, alpha)
